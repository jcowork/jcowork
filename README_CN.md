...
| 平台 | 格式 | 下载 |
|------|------|------|
| macOS (Apple Silicon) | `.dmg` | [Jcowork_0.2.11_aarch64.dmg](https://github.com/jcowork/jcowork/releases/download/v0.2.11/Jcowork_0.2.11_aarch64.dmg) |
| Windows (x64) | `.msi` | [Jcowork_0.2.11_x64_en-US.msi](https://github.com/jcowork/jcowork/releases/download/v0.2.11/Jcowork_0.2.11_x64_en-US.msi) |
| Windows (x64) | `.exe` | [Jcowork_0.2.11_x64-setup.exe](https://github.com/jcowork/jcowork/releases/download/v0.2.11/Jcowork_0.2.11_x64-setup.exe) |
...
| 方法 | 路径 | 认证 | 说明 |
|------|------|------|------|
| POST | `/api/cron-jobs` | 需要 | 创建周期任务（指定名称、Prompt、模型、频次） |
| GET | `/api/cron-jobs/{id}/results` | 需要 | 获取指定任务的执行结果列表 |
| POST | `/api/cron-jobs/{id}/results` | 需要 | 存储任务执行结果 |
...
周期任务是面向 UI 的定时 LLM 调用功能，用户可在「周期任务」页面直接创建和管理周期性任务，无需通过聊天指令。

#### 核心功能

- **任务创建** — 设置任务名称、Prompt 描述，指定执行模型和执行频次
- **模型选择** — 从已配置的 LLM Provider 中选择执行模型，按提供商分组展示
- **执行频次** — 支持三种频次模式：
  - **每小时** — 指定分钟，每小时执行一次
  - **每日** — 指定时:分，每天执行一次
  - **每月** — 指定日期、时:分，每月执行一次
- **执行结果** — 每个任务下方展示可折叠的执行结果列表，包含执行状态（成功/失败）、执行时间和输出内容

#### 使用方式

1. 点击左侧导航栏的「周期任务」进入页面
2. 点击「+ 添加新任务」展开创建表单
3. 填写任务名称、Prompt 描述
4. 从下拉列表中选择执行模型（按 LLM 提供商分组）
5. 选择执行频次（每小时/每日/每月），并设置具体时间
6. 点击「创建任务」完成任务创建
7. 任务列表中的每个任务都有「▶ 执行结果」按钮，点击可展开/折叠该任务的执行历史

#### 与聊天定时任务的区别

| 特性 | 周期任务（UI） | 定时任务（聊天） |
|------|---------------|----------------|
| 创建方式 | Web UI 表单 | 聊天中通过 Agent 工具 |
| 模型选择 | 每个任务可独立指定模型 | 使用当前聊天模型 |
| 执行结果 | UI 中可折叠展示 | 通过通知推送 |
| 适用场景 | 定期报告、数据分析等固定任务 | 临时性定时提醒 |
