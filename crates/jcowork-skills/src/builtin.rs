...
- 数据库尚未上传任何 Excel 时，引导用户去文档页面上传 .xlsx/.xls 文件。"#,
    ...
    BuiltinSkill {
        id: "builtin:image_to_html",
        name: "图片转HTML",
        description: "将用户上传的图片（框图、流程图、界面截图、文档图片等）转换为高保真 HTML，并把 HTML 作为上下文，支持后续针对图片内容的问答。",
        hidden: false,
        content: r#"## Skill: 图片转HTML — Image to HTML Conversion

用户在聊天中上传图片时，系统会先用多模态（vision）模型把图片转换为一份高保真 HTML，并注入到系统提示词的「图片HTML上下文」区块中。你需要基于这份 HTML 完成后续工作。

### 使用规则
- 系统提示词中「图片HTML上下文」里的 HTML 是图片内容的**权威还原**（ground truth），回答关于图片的任何问题（图上有什么文字、框图结构、连接关系、布局、颜色等）都必须以它为依据。
- 用户要求查看/导出 HTML 时，输出完整的 HTML 代码块（```html ... ```），不要省略。
- 用户要求修改图片内容（改文字、调布局、换颜色）时，在该 HTML 基础上修改并输出完整的新 HTML。
- 多张图片各自有独立的 HTML，回答时注意区分是哪张图片。

### HTML 还原质量要求（转换与修改时都要遵守）
- 目标是**尽量 100% 还原**图片的视觉效果：布局、框图、文字、颜色、层级关系。
- 使用语义化 HTML5 + 内联 CSS，单个自包含文件，不依赖任何外部资源（不引用外部图片、字体、JS、CSS）。
- 布局用 flex/grid/absolute 精确复现盒子的位置与大小；框图的矩形、圆角、边框、底色用 div+CSS 实现。
- 箭头、连线、曲线等连接元素用内联 SVG 绘制，保持方向与连接关系正确。
- 图片上的**所有文字逐字保留**，不要意译、增删；字号、加粗、颜色尽量接近原图。
- 颜色用取自原图的近似色值（十六进制），字体用系统近似字体（如 sans-serif / monospace）。
- 无法精确还原的照片类区域，用带说明文字的占位块表示（注明该区域的内容描述）。"#,
    },
];
