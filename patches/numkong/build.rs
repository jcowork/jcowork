...
fn build_numkong() -> Result<HashMap<String, bool>, String> {
    ...
    // Source files
    // MSVC does not support C99 via .std("c99"), but supports /std:c11.
    // Use .std("c11") for MSVC and .std("c99") for other compilers.
    let is_msvc_target = std::env::var("CARGO_CFG_TARGET_ENV").unwrap_or_default() == "msvc";
    if is_msvc_target {
        build.std("c11");
    } else {
        build.std("c99");
    ...
    //
    // On MSVC targets, the cc crate may not propagate the INCLUDE env var to cl.exe.
    // Add system include paths explicitly to ensure intrin.h and other headers are found.
    if is_msvc_target {
        if let Ok(vs_install_dir) = std::env::var("VCINSTALLDIR") {
            // VCINSTALLDIR is set by vcvarsall.bat
            let _ = vs_install_dir; // suppress unused warning
        } else {
            // Try to find MSVC include paths manually
            let msvc_base = r"C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC";
            let sdk_base = r"C:\Program Files (x86)\Windows Kits\10\Include";
    ...
    // On MSVC, source files are UTF-8 but the compiler defaults to the system
    // code page (e.g. GBK/CP936 on Chinese Windows). The /utf-8 flag sets both
    // source and execution character sets to UTF-8, preventing misparse of
    // Unicode characters in comments and string literals.
    if is_msvc_target {
        build.flag("/utf-8");
    }
